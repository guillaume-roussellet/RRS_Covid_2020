...
// Computes the sqrt of the covariance matrix
Eigen::MatrixXd Eigenvalue_decomp(const MatrixXd A){
  
  Eigen::EigenSolver<MatrixXd> eig(A);
  Eigen::MatrixXcd Vec = eig.eigenvectors();
  Eigen::VectorXcd Val = eig.eigenvalues();
  
  Eigen::VectorXd Real_val = Val.real();
  Eigen::MatrixXd Real_vec = Vec.real();
  
  
  for(int i=0; i < Real_val.size(); i++){
    if(Real_val(i) < 0){Real_val(i) = 0;}
  }
  
  Eigen::VectorXd val_sqrt = Real_val.array().sqrt();
  Eigen::MatrixXd diag_val = val_sqrt.asDiagonal();
  //std::cout << Real_val << std::endl;
  
  
  Eigen::MatrixXd Sqrt_A = Real_vec * diag_val * Real_vec.completeOrthogonalDecomposition().pseudoInverse();
  
  //std::cout << Sqrt_A << std::endl;
...
// [[Rcpp::export]]
Rcpp::List EKF_filter_cpp_counter(const Rcpp::List      & Parameters,
                                  ...
                                  const Eigen::MatrixXd & Dummy_theta_F_counter,
                                  const Eigen::MatrixXd & Dummy_theta_I_counter,
                                  const Eigen::MatrixXd & Dummy_masks_counter){
  ...
  Eigen::MatrixXd AR_star             = State_Space("AR_star");
  Eigen::MatrixXd Mean_mult_star      = State_Space("Mean_mult_star");
  ...
  Eigen::MatrixXd Mult_Mat_Meas_Eq                  = Eigen::MatrixXd::Zero(n_W, 1 * n_states);
  ...
  Eigen::MatrixXd res_t          = Eigen::MatrixXd::Zero(n_states, time);
  ...
  //--------------------
  // Constructing Theta matrices
  Eigen::MatrixXd theta_masks_counter     = Ones_Matrix * mask_reduction + Dummy_masks_counter * (1 - mask_reduction);
  Eigen::MatrixXd theta_I_matrix_counter  = Ones_Matrix * Diag_theta_I_low + Dummy_theta_I_counter * (Diag_theta_I_high - Diag_theta_I_low);
  Eigen::MatrixXd theta_F_matrix_counter  = Ones_Matrix * Diag_theta_F_low + Dummy_theta_F_counter * (Diag_theta_F_high - Diag_theta_F_low);
  
  // Elements that will be updated in the recursions
  //------------------------------------------------
  Eigen::VectorXd theta_F_vector_counter    = theta_F_matrix_counter.row(0);
  Eigen::VectorXd theta_I_vector_counter    = theta_I_matrix_counter.row(0);
  Eigen::VectorXd theta_mask_vector_counter = theta_masks_counter.row(0);
  // COMMUTING MATRICES
  Eigen::MatrixXd Weight_commut_t_counter   = Weight_commuting * Diag_theta_I_high;
  Eigen::MatrixXd Diag_mat_commut_counter   = (Weight_commut_t_counter * Ones_vector).asDiagonal();
  Eigen::MatrixXd Omega_commut_counter      = tau_com * Weight_commut_t_counter.transpose() - tau_com * Diag_mat_commut_counter;
  // TRAVEL MATRICES
  Eigen::MatrixXd Weight_travel_t_counter   = tau_trav * Weight_travel * Diag_theta_I_high * Diag_theta_F_high;
  Eigen::MatrixXd Diag_mat_travel_counter   = (Weight_travel_t_counter * Ones_vector).asDiagonal();
  Eigen::MatrixXd Omega_travel_counter      = Weight_travel_t_counter.transpose() - Diag_mat_travel_counter;
  // DEFINE OMEGA(t)
  Eigen::MatrixXd Omega_t_counter           = Omega_commut_counter + Omega_travel_counter ;
  

  // Jacobian
  Eigen::MatrixXd Jacobian_vcov_counter   = Eigen::MatrixXd::Zero(n_states, n_W);

  // Separating the different components
  Eigen::ArrayXd D_counter            = W.segment(5 * n_states, n_states);
  Eigen::VectorXd S_counter           = W.segment(6 * n_states, n_states);
  Eigen::VectorXd I_counter           = W.segment(7 * n_states, n_states);
  Eigen::ArrayXd R_counter            = W.segment(8 * n_states, n_states);

  
  
  // Building the nonlinear part of the mean vector
  Eigen::ArrayXd Commut_Travel_S_counter  = ((Id + Omega_t_counter) * S_counter).array();
  Eigen::ArrayXd Commut_Travel_I_counter  = ((Id + Omega_t_counter) * I_counter).array();
  Eigen::ArrayXd Nonlinear_arr_counter    = theta_I_high.array() * Normalized_Betas * Commut_Travel_S_counter * Commut_Travel_I_counter;
  Eigen::VectorXd Nonlinear_vec_counter   = Nonlinear_arr_counter;
  
  
  // Building the nonlinear part in the variance-covariance matrix
  Eigen::VectorXd First_vec_Theta_TV_counter      = theta_I_vector_counter.array() * theta_mask_vector_counter.array() * Normalized_Betas;
  Eigen::MatrixXd First_Mat_Theta_TV_counter      = First_vec_Theta_TV_counter * First_vec_Theta_TV_counter.transpose();
  Eigen::MatrixXd Second_Mat_Theta_TV_counter     = D_function(Weight_commut_t_counter, Weight_travel_t_counter, 
                                                               S_counter, I_counter, tau_com, 1, Omega_t_counter);
  Eigen::MatrixXd First_row_Theta_TV_counter      = First_Mat_Theta_TV_counter.array() * Second_Mat_Theta_TV_counter.array();
  
  Eigen::MatrixXd Diag_mult_Theta_TV_var_counter  = Nonlinear_vec_counter.asDiagonal();
  Eigen::MatrixXd Theta_TV_var_counter            = First_row_Theta_TV_counter + Diag_mult_Theta_TV_var_counter;
  
  
  //----------------------------------------------
  // Constructing the full model objects
  //----------------------------------------------
  Eigen::VectorXd INTERCEPT                         = Eigen::VectorXd::Zero(n_W);
  Eigen::MatrixXd AR_MATRIX                         = Eigen::MatrixXd::Zero(n_W, n_W);
  INTERCEPT.segment(0, 5 * n_states)                = Intercept_full;
  INTERCEPT.segment(5 * n_states, 4 * n_states)     = Intercept_full.segment(0, 4 * n_states);
  AR_MATRIX.block(0,0, 5 * n_states, 5 * n_states)  = AR_full;
  AR_MATRIX.block(5 * n_states, 5 * n_states, 4 * n_states, 4 * n_states)  = AR_full.block(0,0, 4 * n_states, 4 * n_states);

  Eigen::MatrixXd MEAN_MULT_BASE                    = Eigen::MatrixXd::Zero(n_W, n_states);
  Eigen::MatrixXd MEAN_MULT_COUNTER                 = Eigen::MatrixXd::Zero(n_W, n_states);
  MEAN_MULT_BASE.block(0,0, 5 * n_states, n_states) = Mean_mult_full;
  MEAN_MULT_COUNTER.block(5 * n_states, 0, 4 * n_states, n_states) = Mean_mult_full.block(0, 0, 4 * n_states, n_states);



  // Initializing the complete vcov
  //-------------------------------
  Eigen::MatrixXd vcov_first_block  = Eigen::MatrixXd::Zero(5 * n_states, 5 * n_states);
  Eigen::MatrixXd vcov_second_block = Eigen::MatrixXd::Zero(5 * n_states, 5 * n_states);
  Eigen::MatrixXd sqrt_first_block  = Eigen::MatrixXd::Zero(5 * n_states, 5 * n_states);
  Eigen::MatrixXd sqrt_second_block = Eigen::MatrixXd::Zero(5 * n_states, 5 * n_states);

  Eigen::MatrixXd CONDI_VAR_TOTAL   = Eigen::MatrixXd::Zero(n_W, n_W);
  Eigen::MatrixXd SQRT_CONDI_VAR    = Eigen::MatrixXd::Zero(n_W, 5 * n_states);
  ...
  //for(int t = 0; t < 3; t++){
    ...
    // SECOND BLOCK
    //------------------------------------
    theta_F_vector_counter    = theta_F_matrix_counter.row(t);
    theta_I_vector_counter    = theta_I_matrix_counter.row(t);
    theta_mask_vector_counter = theta_masks_counter.row(t);
    // COMMUTING MATRICES
    Weight_commut_t_counter   = Weight_commuting * theta_I_vector_counter.asDiagonal();
    Diag_mat_commut_counter   = (Weight_commut_t_counter * Ones_vector).asDiagonal();
    Omega_commut_counter      = tau_com * Weight_commut_t_counter.transpose() - tau_com * Diag_mat_commut_counter;
    // TRAVEL MATRICES
    Weight_travel_t_counter   = tau_trav * Weight_travel *  theta_I_vector_counter.asDiagonal() * theta_F_vector_counter.asDiagonal();
    Diag_mat_travel_counter   = (Weight_travel_t_counter * Ones_vector).asDiagonal();
    Omega_travel_counter      = Weight_travel_t_counter.transpose() - Diag_mat_travel_counter;
    // DEFINE OMEGA(t)
    Omega_t_counter           = Omega_commut_counter + Omega_travel_counter ;
    ...
    // Building the nonlinear part of the mean vector
    //-----------------------------------------------
    Commut_Travel_S_counter   = ((Id + Omega_t_counter) * S_counter).array();
    Commut_Travel_I_counter   = ((Id + Omega_t_counter) * I_counter).array();
    Nonlinear_arr_counter     = theta_I_vector_counter.array() * theta_mask_vector_counter.array() * 
      Normalized_Betas * Commut_Travel_S_counter * Commut_Travel_I_counter;
    Nonlinear_vec_counter     = Nonlinear_arr_counter;
    
    
    // Building the nonlinear part in the variance-covariance matrix
    //--------------------------------------------------------------
    First_vec_Theta_TV_counter      = theta_I_vector_counter.array() * theta_mask_vector_counter.array() * Normalized_Betas;
    First_Mat_Theta_TV_counter      = First_vec_Theta_TV_counter * First_vec_Theta_TV_counter.transpose();
    Second_Mat_Theta_TV_counter     = D_function(Weight_commut_t_counter, Weight_travel_t_counter, 
                                                                 S_counter, I_counter, tau_com, 1, Omega_t_counter);
    First_row_Theta_TV_counter      = First_Mat_Theta_TV_counter.array() * Second_Mat_Theta_TV_counter.array();
    
    Diag_mult_Theta_TV_var_counter  = Nonlinear_vec_counter.asDiagonal();
    Theta_TV_var_counter            = First_row_Theta_TV_counter + Diag_mult_Theta_TV_var_counter;
    ...
    W_pred = INTERCEPT + AR_MATRIX * W + MEAN_MULT_BASE * Nonlinear_vec + MEAN_MULT_COUNTER * Nonlinear_vec_counter;



    // Conditional variance matrix
    // Jacobian first block
    Eigen::VectorXd Jacobian_Block_1  = theta_I_vector.array() * theta_mask_vector.array() * Normalized_Betas * Commut_Travel_I;
    Eigen::VectorXd Jacobian_Block_2  = theta_I_vector.array() * theta_mask_vector.array() * Normalized_Betas * Commut_Travel_S;
    Eigen::VectorXd Jacobian_Block_3  = theta_I_vector.array() * theta_mask_vector.array()/Pop_vector * Commut_Travel_I * Commut_Travel_S;
    ...
    // Jacobian second block of variables
    Eigen::VectorXd Jacobian_Block_1_counter  = theta_I_vector_counter.array() * theta_mask_vector_counter.array() * 
      Normalized_Betas * Commut_Travel_I_counter;
    Eigen::VectorXd Jacobian_Block_2_counter  = theta_I_vector_counter.array() * theta_mask_vector_counter.array() * 
      Normalized_Betas * Commut_Travel_S_counter;
    Eigen::VectorXd Jacobian_Block_3_counter  = theta_I_vector_counter.array() * theta_mask_vector_counter.array()/Pop_vector * 
      Commut_Travel_I_counter * Commut_Travel_S_counter;

    Eigen::MatrixXd Diag_Jacobian_Block_1_counter = Jacobian_Block_1_counter.asDiagonal();

    Jacobian_vcov_counter                                             = 0 * Jacobian_vcov;
    Jacobian_vcov_counter.block(0, 6 * n_states, n_states, n_states)  = (Id + Omega_t_counter) * Jacobian_Block_1_counter.asDiagonal();
    Jacobian_vcov_counter.block(0, 7 * n_states, n_states, n_states)  = (Id + Omega_t_counter) * Jacobian_Block_2_counter.asDiagonal();
    Jacobian_vcov_counter.block(0, 4 * n_states, n_states, n_states)  = Jacobian_Block_3_counter.asDiagonal() * Premult_Beta;;


    Total_jacobian = AR_MATRIX + MEAN_MULT_BASE * Jacobian_vcov + MEAN_MULT_COUNTER * Jacobian_vcov_counter;


    // Conditional VARIANCE OF THE STATE
    //-----------------------------------
    vcov_first_block  = 0 * vcov_first_block;
    vcov_second_block = 0 * vcov_second_block;
    sqrt_first_block  = 0 * sqrt_first_block;
    sqrt_second_block = 0 * sqrt_second_block;
    SQRT_CONDI_VAR    = 0 * SQRT_CONDI_VAR;

    vcov_first_block  = Condi_var(Parameters, X, I, Variance_AR_full, Theta_TV_var);
    vcov_second_block = Condi_var(Parameters, X, I_counter, Variance_AR_full, Theta_TV_var_counter);


    sqrt_first_block  = Eigenvalue_decomp(vcov_first_block);
    sqrt_second_block = Eigenvalue_decomp(vcov_second_block);

    SQRT_CONDI_VAR.block(0, 0, 5 * n_states, 5 * n_states) = sqrt_first_block;
    SQRT_CONDI_VAR.block(5 * n_states, 0, 4 * n_states, 5 * n_states) = sqrt_second_block.block(0,0,4 * n_states, 5 * n_states);


    // Predict the variance-covariance matrix
    P_pred = Total_jacobian * P * Total_jacobian.transpose() + SQRT_CONDI_VAR * SQRT_CONDI_VAR.transpose();
    ...
    Eigen::VectorXd I_ToModif_2 = W_up.segment(7 * n_states, n_states);
    ...
    W_up.segment(7 * n_states, n_states)    = I_ToModif_2;
    ...
    D_counter       = W.segment(5 * n_states, n_states);
    S_counter       = W.segment(6 * n_states, n_states);
    I_counter       = W.segment(7 * n_states, n_states);
    R_counter       = W.segment(8 * n_states, n_states);

    Normalized_Betas = Betas.array()/Pop_vector;
